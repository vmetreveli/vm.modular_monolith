...
public class DeleteDiscountCommandHandler(IDiscountRepository discountRepository) : ICommandHandler<DeleteDiscountCommand, bool>
{

    public async Task<bool> Handle(DeleteDiscountCommand request, CancellationToken cancellationToken = default) =>
        await discountRepository.DeleteDiscount(request.ProductName, cancellationToken);
}